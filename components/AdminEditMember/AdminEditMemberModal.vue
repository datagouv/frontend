...
import type { EditingMember, Member, MemberRole } from '~/types/types';
...
async function updateRole(member: EditingMember) {
  ...
  try {
    loading.value = true;
    await $api(`/api/1/organizations/${props.oid}/member/${member.user.id}`, { method: 'PUT', body: JSON.stringify({ role: member.newRole }) })
    emits('memberUpdated');
    closeModal();
  } catch (e) {
...
async function removeMemberFromOrganization(member: Member) {
  try {
    loading.value = true;
    await $api(`/api/1/organizations/${props.oid}/member/${member.user.id}`, { method: 'DELETE' })
    emits('memberUpdated');
  } catch (e) {
