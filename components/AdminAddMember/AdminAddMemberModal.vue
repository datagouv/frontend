...
import type { MemberRole, MultiSelectOption } from '~/types/types';
import useUserAvatar from '~/utils/auth';
...
async function submit() {
  try {
    loading.value = true;
    if(userId.value) {
      await $api(`/api/1/organizations/${props.oid}/member/${userId.value}`, { method: 'POST', body: JSON.stringify({ role: role.value }) })
      emits('memberAdded');
      closeModal();
    }
  } catch (e) {
