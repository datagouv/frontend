<template>
    ...
    <Breadcrumb>
      ...
      <div class="fr-grid-row fr-grid-row--gutters fr-grid-row--middle justify-center">
        ...
        <div class="fr-col-auto" v-if="canEdit">
      ...
      <AdminTable class="fr-table--layout-fixed" :loading>
        <thead>
          <tr>
            ...
            <AdminTableTh scope="col" v-if="canEdit">{{ t("Action") }}</AdminTableTh>
          </tr>
        </thead>
        <tbody>
  ...
  import type { AdminBadgeState, EditingMember, MemberRole, PendingMembershipRequest } from "~/types/types";
  ...
  const canEdit = computed(() => isAdmin(me.value) || members.value.some(member => member.user.id === me.value.id && member.role === "admin"));
